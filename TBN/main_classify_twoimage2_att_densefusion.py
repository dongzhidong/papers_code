...
class MyNet(nn.Module):
    def __init__(self,out_dim,bands,seg_bands,patch_size=1,batch_size=128):
        ...
        self.b1_conv3d1=nn.Sequential(
            nn.Conv3d(1,8,kernel_size=(7,3,3),padding=1),
            nn.BatchNorm3d(8),
            nn.ReLU()
        )
        self.b1_conv3d2 = nn.Sequential(
            nn.Conv3d(8, 16, kernel_size=(5,3, 3),padding=1),
            nn.BatchNorm3d(16),
            nn.ReLU()
        )
        self.b1_conv3d3 = nn.Sequential(
            nn.Conv3d(16, 32, kernel_size=(3, 3, 3),padding=1),
            nn.BatchNorm3d(32),
            nn.ReLU(),
        )
        self.b2_conv3d1=nn.Sequential(
            nn.Conv3d(1,8,kernel_size=(7,3,3),padding=1),
            nn.BatchNorm3d(8),
            nn.ReLU()
        )
        self.b2_conv3d2 = nn.Sequential(
            nn.Conv3d(8, 16, kernel_size=(5,3, 3),padding=1),
            nn.BatchNorm3d(16),
            nn.ReLU()
        )
        self.b2_conv3d3 = nn.Sequential(
            nn.Conv3d(16, 32, kernel_size=(3, 3, 3),padding=1),
            nn.BatchNorm3d(32),
            nn.ReLU(),
        )
    ...
    def forward(self,X,XS):
        X=self.b1_conv3d1(X)
        XS=self.b2_conv3d1(XS)
        X.add_(XS)
        X=self.b1_conv3d2(X)
        XS=self.b2_conv3d2(XS)
        X.add_(XS)
        X=self.b1_conv3d3(X)
        XS=self.b2_conv3d3(XS)
        X.add_(XS)
        # X=self.conv3dseq(X)
...
class Args():
    def __init__(self):
        self.method_name='twoBranch_att_denF'
        #
