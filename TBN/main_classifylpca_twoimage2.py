...
def load_data(data_name,nC=50,show=False):
    ...
    if pcaseg:
        seg_name= data_name + '_pca255seg' + str(nC) + '.mat'
    ...
    Seg_img=Seg_img+1
    if pcaseg:
        Seg_img=np.expand_dims(Seg_img,axis=2).repeat(image.shape[-1],axis=2)
    if show:
        plt.subplot(131)
...
def test_and_show(args,mydata=None,colormap=None):
    ...
    if mydata is None:
        ...
        mydata = MyData(image_pca, seg_img_pca, labels_raw, train_ind, test_ind, val_ind, args.patch_size,
                        args.batch_size)
