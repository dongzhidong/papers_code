...
# @Software: PyCharm
# @Purpose :this is for two-image-Net with attention mechainism
...
class MyNet(nn.Module):
    def __init__(self,out_dim,bands,seg_bands,patch_size=1,batch_size=128):
        ...
        self.bna1=nn.BatchNorm1d(fc_in)
        self.bna2=nn.BatchNorm1d(fc_in)
    ...
    def forward(self,X,XS):
        ...
        XS=self.conv3dseq2(XS)
        XS=XS.view(self.f_shape1)
        XS=self.conv2d2(XS)
        XS = XS.view(self.batch_size, -1)
        XS_a=self.att2(XS)
        # XS=self.bna2(XS*XS_a)
        XS=XS*XS_a
    ...
    def get_attention_nums(self):
        ...
        # XS = torch.ones(self.batch_size, 1, self.bands, self.patch_size, self.patch_size)
        # XS = self.conv3dseq2(XS)
        # XS = XS.view(self.f_shape1)
        # XS = self.conv2d2(XS)
        # XS = XS.view(self.batch_size, -1)
...
def test_and_show(mydata=None,colormap=None):
    ...
    # load data
    if mydata is None:
        image_raw, labels_raw, seg_img = load_data(args.data_name)
    ...
    myNet = MyNet(args.classnums, bands_num, bands_seg_num, args.patch_size, args.batch_size)
    myNet.to(device)
    ...
    for iter in tqdm(range(iters),desc='test all:'):
        X, XS, Y ,ind= mydata.get_all(iter)
...
def test_val(mydata,myNet,fortype='test'):
    ...
    for iter in range(iters):
        ...
        # X, XS, Y = mydata.get_batch_oneimage(iter,'seg',fortype)
        X = np.transpose(X, [0, 3, 1, 2])
        XS = np.transpose(XS, [0, 3, 1, 2])
        X = torch.from_numpy(X).requires_grad_(True).to(device)
        XS = torch.from_numpy(XS).requires_grad_(True).to(device)
...
def run(args=None):
    ...
    for epoch in range(args.epochs):
        ...
        for iter in range(mydata.train_iters):
            ...
            X,XS,Y=mydata.get_batch_twoimage(iter)
...
class Args():
    def __init__(self, data_name='Salinas'):
        self.method_name = 'twoBranch_att_'
        #
        ...
        self.record = True
        self.recordFile = 'twoBranchHouston_att_results.txt'
...
if __name__ == '__main__':
    ...
    # exit()
    # args=Args()
    # args.method_name = args.method_name + '_p{}_nC{}_'.format(args.patch_size, args.base_nC)
    # run_more(args)
    ...
    for patch in patch_grid:
        for nC in nC_grid:
            if patch==7 and nC in [30,50,70,100]:
                print('pass p nC',patch, ' ',nC)
                continue
            args = Args()
