...
class myCNN(nn.Module):
    def __init__(self,input_channels,n_classes,patch_size,img_channel):
        ...
        self.conv1=nn.Conv3d(1,128,(3,3,3),padding=(0,1,1))
        self.bn2 = nn.BatchNorm3d(64)
        self.pool=nn.MaxPool3d(3, stride=1,padding=1)
        self.avgpool=nn.AvgPool3d(3, stride=1,padding=1)
        self.conv2=nn.Conv3d(128,64,(3,3,3),padding=(0,1,1))
        self.bn3 = nn.BatchNorm3d(16)
        self.conv3=nn.Conv3d(64,16,(3,3,3),padding=(0,1,1))

        self.bn21 = nn.BatchNorm3d(128)
        self.conv21=nn.Conv3d(1,128,(3,3,3),padding=(0,1,1))
        self.bn22 = nn.BatchNorm3d(64)
        # self.pool=nn.MaxPool3d(3, stride=1,padding=1)
        self.conv22=nn.Conv3d(128,64,(3,3,3),padding=(0,1,1))
        self.bn23 = nn.BatchNorm3d(16)
        self.conv23=nn.Conv3d(64,16,(3,3,3),padding=(0,1,1))
    ...
    def forward(self, x):
        ...
        x1 = torch.cat((self.pool(x1),self.avgpool(x1)),2)
        ...
        x2 =  torch.cat((self.pool(x2),self.avgpool(x2)),2)
    ...
    def get_fc_size(self,chanels):
        with torch.no_grad():
            ...
            x = torch.cat((self.pool(x), self.avgpool(x)), 2)
...
def test(test_loader,cnnNet,**kwargs):
    ...
    aa = np.mean(acc_for_each_class)
    kp = metrics.cohen_kappa_score(labels, preds)
    classify_report = metrics.classification_report(labels, preds)
    return oa,aa,kp,acc_for_each_class,classify_report
...
def train(train_loader,val_loader,test_loader,**kwargs):
    ...
    lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=30, verbose=True,min_lr=5.0e-5)
    ...
    for epoch in range(epochs):
        ...
        if early_cnt>early_stop:
            break
    test_oa,test_aa,test_kp,acc_for_each_class,report=test(test_loader,cnnNet,**kwargs)
...
def record_results(filename,oa,aa,kappa,):
...
if __name__ == '__main__':
    ...
    for run_id in range(run_nums):
        ...
        val_ind, _, _, _ = tools.my_train_test_split(test_ind, labels[test_ind[:, 0], test_ind[:, 1]], train_rate=0.2,
                                                     random_state=None)
        # 构造数据集
        ...
        with open('results.txt', 'a') as f:
            ...
            f.write('run_id {}/{} | best model oa:{:.6f} aa:{:.6f} kappa:{:.6f}\n'.format(run_id+1,run_nums,oa,aa,kp))
    ...
    with open('results.txt', 'a') as f:
        f.write('*********************\n')
        f.write(time.asctime() + ' ')
        f.write('summary oa:{}({} aa:{}({}) kappa:{}({}))\n'.format(np.mean(oa_list),np.std(oa_list),np.mean(aa_list),np.std(aa_list),
          np.mean(kp_list),np.std(kp_list)))
        f.write('\n\n\n\n')
    print('end')
