...
class MyNet(nn.Module):
    def __init__(self,out_dim,bands,seg_bands,patch_size=1,batch_size=128):
        ...
        self.attnum = torch.ones([1,1],dtype=torch.float32).to(device)
        self.att1=nn.Sequential(
            nn.Linear(1,fc_in,bias=False),
            nn.Sigmoid()
        )
...
class Args():
    def __init__(self, data_name='Salinas'):
        ...
        self.record = True
        self.recordFile = 'twoBranchnewatt_IP_results.txt'
