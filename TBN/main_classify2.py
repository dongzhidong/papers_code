...
# @Purpose :this is  for one-image-net
...
class MyData():
    def __init__(self,image,seg_image,labels,train_ind,test_ind,val_ind=[],patch_size=1,batch_size=128):
        w, h, b = image.shape
...
#     def __init__(self,out_dim,patch_size=1,batch_size=128):
...
#
#         )
#         self.fc_nums=self.get_fea_nums()
#         self.seq2=nn.Sequential(
#             nn.Linear(self.fc_nums,1024),
#             nn.BatchNorm1d(1024),
#             nn.ReLU(),
#
...
#         # X=self.seq(X)
#         # X=self.seq2(X)
#         return self.seq2(self.seq(X).view(self.batch_size,-1))
#
#     def get_fea_nums(self):
#         x=torch.ones(self.batch_size,1,self.patch_size,self.patch_size)
#         x=self.seq(x)
#         a,_=x.view([-1,self.batch_size]).shape
#         return a

class MyOneNet(nn.Module):
    def __init__(self,out_dim,bands,patch_size=1,batch_size=128):
        super(MyOneNet,self).__init__()
        self.patch_size=patch_size
        self.batch_size=batch_size
        self.bands=bands
...
def test_val(mydata,myNet,fortype='test'):
    ...
    for iter in range(iters):
        # X, Y = mydata.get_batch_oneimage(iter,'image',fortype)
        X, Y ,ind= mydata.get_batch_oneimage(iter,args.imgtype,fortype)
...
def test_and_show(mydata=None,colormap=None):
    ...
    if mydata is None:
        ...
        mydata = MyData(image_pca, seg_img_pca, labels_raw, train_ind, test_ind, val_ind, args.patch_size, args.batch_size)


    bands_num = mydata.image_shape[-1]
    bands_seg_num = mydata.seg_image_shape[-1]
    if args.imgtype=='image':
        bands=bands_num
    else:
        bands=bands_seg_num
    myOneNet = MyOneNet(args.classnums, bands, args.patch_size, args.batch_size)
    myOneNet.to(device)
    ...
    for iter in tqdm(range(iters),desc='test all:'):
        X, Y,ind = mydata.get_all(iter, args.imgtype)
        X = np.transpose(X, [0, 3, 1, 2])
        X = torch.from_numpy(X).requires_grad_(True).to(device)
...
def run(args):
    ...
    remain_index=np.where(labels_raw!=0)
    remain_index=np.array(list(zip(remain_index[0],remain_index[1])))
    train_ind,test_ind,_,_=my_train_test_split(remain_index,labels_raw[remain_index[:,0],remain_index[:,1]],args.train_size,random_state=seed)
    ...
    image_pca = image_raw
    image_pca=preprocess(image_pca)
    seg_img_pca=preprocess(seg_img_pca,1)
    ...
    myOneNet = MyOneNet(args.classnums,bands_num, args.patch_size, args.batch_size)
    myOneNet.to(device)

    criterion=nn.CrossEntropyLoss()
    optim=torch.optim.SGD(myOneNet.parameters(), lr=0.0005, momentum=0.9)
    # optim=torch.optim.SGD(myNet.parameters(), lr=0.0005, momentum=0.9)
    # optim = torch.optim.Adagrad(myOneNet.parameters(), lr=0.0005, weight_decay=0.01)

    # print("Model's state_dict:")
    # # Print model's state_dict
    # for param_tensor in myOneNet.state_dict():
    #     print(param_tensor, "\t", myOneNet.state_dict()[param_tensor].size())
    ...
    for epoch in range(args.epochs):
        ...
        for iter in range(mydata.train_iters):
            ...
            # one image,
            # X,Y=mydata.get_batch_oneimage(iter,'image')
            X,Y,ind=mydata.get_batch_oneimage(iter,args.imgtype)
            X=np.transpose(X,[0,3,1,2])
            X=torch.from_numpy(X).requires_grad_(True).to(device)
            Y=torch.from_numpy(Y).long().to(device)
            X=X.unsqueeze(1)
        ...
        if (epoch + 1) % 5 == 0:
            ...
            val_acc, _, _ = test_val(mydata, myOneNet, 'val')
            ...
            if val_acc >= val_bst:
                ...
                bst_epoch = epoch + 1
                modelname = args.method_name + '_' + args.data_name + '_valbst' + '.model'
                torch.save(myOneNet.state_dict(), os.path.join(args.model_save_path, modelname))
                print('saving model val_acc: ',val_acc,' bst_epoch: ',bst_epoch,'bst_acc: ',val_bst)
            # test on best model
    modelname = args.method_name + '_' + args.data_name + '_valbst' + '.model'
    myOneNet.load_state_dict(torch.load(os.path.join(args.model_save_path, modelname)))
    test_acc, y_pred, y_true = test_val(mydata, myOneNet, 'test')
    print('test_acc:', test_acc)
...
# imgtype='image'  # image seg
# imgtype='seg'  # image seg
class Args():
    def __init__(self):
        self.method_name = 'OneBranch'
        self.imgtype='seg'
        self.method_name=self.method_name+"_"+self.imgtype
        ...
        self.record = True
        self.recordFile = 'OneBranch'+self.imgtype+'_results.txt'
