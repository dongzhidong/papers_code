#!/usr/bin/python3.8
# -*- coding: utf-8 -*-
# @Time    : 2021/12/29 10:08
# @Author  : dongdong
# @File    : main_classify.py
# @Software: PyCharm
# @Purpose :this is for single branch net HSIC with adaptive label 
...
# python ./MT/MTnet_justCEone.py --data_name IndianPines --batch_size 1024 --train_size 1 
...
def run(args=None):
    ...
    # in the "*justCE" experiment ,we discard first stage 
    ...
    for epoch in range(args.epochs):
        myNet.train()
        mydata.shuffle()
        for iter in range(mydata.all_iters):
            optim.zero_grad()
            X1, Yc,ind = mydata.get_batch(iter,'all')
            Yc1=np.zeros((Yc.shape[0],),dtype=np.float32)-1
            for ind_i, ind_case in enumerate(ind):
                if ind_case in mydata.train_ind:
                    Yc1[ind_i]=Yc[ind_i]
            X1 = np.transpose(X1, [0, 3, 1, 2])
            X1 = torch.from_numpy(X1).requires_grad_(True).to(device)
            X1 = X1.unsqueeze(1)
            ...
            Yc1[tmp]=np.argmax(output.detach().cpu(),axis=1)[tmp]
