...
# @Software: PyCharm
# @Purpose :this is for two-stage Multi-task HSIC with adaptive label 
...
def run(args=None):
    ...
    for epoch in range(args.epochs):
        ...
        for iter in range(mydata.all_iters):
            ...
            tmp=np.where(Yc1<0)
            Yc1[tmp]=np.argmax(output.detach(),axis=1)[tmp]
            loss=CE(output, Yc1) 
            loss.backward()
            optim.step()
            logtext='\repoch_cla:{}/{} iter:{}/{} loss:{:.6f}'.format(epoch, args.epochs, iter, mydata.reg_iters,loss.detach().cpu())
            print(logtext,end='')
            # writelogtxt(args.logFile,logtext[1:])
        logtext='\repoch_cla:{}/{} iter:{}/{} loss:{:.6f}'.format(epoch, args.epochs, iter, mydata.reg_iters,loss.detach().cpu())
        print(logtext,end='')
