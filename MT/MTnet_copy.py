...
class MyData():
    ...
    def get_batch_reg(self,iter):
        ...
        Yr=np.zeros((self.batch_size,),dtype=np.float32)
...
def run(args=None):
    ...
    MSE=nn.MSELoss()
    optim = torch.optim.SGD(myNet.parameters(), lr=0.01, momentum=0.9)
    ...
    for epoch in range(args.epochs):
        ...
        for iter in range(mydata.reg_iters):
            ...
            loss.backward()
            optim.step()
            print('\repoch:{}/{} iter:{}/{} loss:{:.6f} lossr:{:.6f} loss1:{:.6f} loss2:{:.6f}'\
                .format(epoch, args.epochs, iter, mydata.reg_iters,loss.detach().cpu(), \
                    lossr.detach().cpu(),loss1.detach().cpu(),loss2.detach().cpu()),end='')
        print('\repoch:{}/{} iter:{}/{} loss:{:.6f} lossr:{:.6f} loss1:{:.6f} loss2:{:.6f}'\
            .format(epoch, args.epochs, iter, mydata.reg_iters,loss.detach().cpu(), \
                lossr.detach().cpu(),loss1.detach().cpu(),loss2.detach().cpu()),end='')
