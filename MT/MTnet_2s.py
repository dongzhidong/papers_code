...
# @Software: PyCharm
# @Purpose :this is for two-stage Multi-task HSIC
...
# python ./MT/MTnet_2s.py --data_name IndianPines --batch_size 1024 --train_size 5 
...
def run(args=None):
    ...
    for epoch in range(args.epochs):
        ...
        for iter in range(mydata.train_iters):
            optim.zero_grad()
            X1, Yc1,ind = mydata.get_batch(iter,'train')
            ...
            loss.backward()
            optim.step()
            logtext='\repoch_cla:{}/{} iter:{}/{} loss:{:.6f}'.format(epoch, args.epochs, iter, mydata.train_iters,loss.detach().cpu())
            print(logtext,end='')
            # writelogtxt(args.logFile,logtext[1:])
        logtext='\repoch_cla:{}/{} iter:{}/{} loss:{:.6f}'.format(epoch, args.epochs, iter, mydata.train_iters,loss.detach().cpu())
        print(logtext,end='')
