...
def test_val(mydata,myNet,fortype='test'):
    ...
    for iter in range(iters):
        ...
        X, Y ,ind= mydata.get_batch_oneimage(iter,imgtype,fortype)
...
def test_and_show(mydata=None,colormap=None):
    ...
    for iter in tqdm(range(iters),desc='test all:'):
        X, Y,ind = mydata.get_all(iter, imgtype)
        X = np.transpose(X, [0, 3, 1, 2])
        X = torch.from_numpy(X).requires_grad_(True).to(device)
...
def run():
    ...
    optim = torch.optim.Adagrad(myOneNet.parameters(), lr=0.0005, weight_decay=1e-6)
    ...
    for epoch in range(args.epochs):
        ...
        for iter in range(mydata.train_iters):
            ...
            X,Y,ind=mydata.get_batch_oneimage(iter,imgtype)
            X=np.transpose(X,[0,3,1,2])
            X=torch.from_numpy(X).requires_grad_(True).to(device)
            Y=torch.from_numpy(Y).long().to(device)
            X=X.unsqueeze(1)
...
class Args():
    def __init__(self):
        self.method_name = 'HybridSN'
        self.method_name=self.method_name+"_image"
        ...
        # self.classnums = 16
        # self.base_nC = 50
        # self.method_name = self.method_name + "_"
