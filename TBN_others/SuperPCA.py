...
# @Software: PyCharm
# the result get by Matlab program, we just use this python file to plot figure.
...
if data_name=='IndianPines':
    img=sio.loadmat(os.path.join(Dataset_path,'Indian_predict_labels'))['predict_labels']
else:
    img = sio.loadmat(os.path.join(Dataset_path, data_name+'_predict_labels'))['predict_labels']
labels_show=img
