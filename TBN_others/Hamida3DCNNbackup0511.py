...
def test_and_show(mydata=None,colormap=None,test_all_fig=False):
    # device = 'cuda:0' if torch.cuda.is_available() else 'cpu'
    args = Args()
    ...
    if mydata is None:
        ...
        val_ind, _ = train_test_split(test_ind, train_size=0.2, random_state=seed)
        if test_all_fig==True:
            print('test_all_fig...')
    ...
    if test_all_fig:
        figname=modelname.split('.')[0] + "_single_allfig.png"
        print('plot all fig : ',figname )
...
def run(args):
    ...
    for epoch in range(args.epochs):
        ...
        print('\repoch:{}/{} iter:{}/{} loss:{:.6f}'.format(epoch, args.epochs, iter, mydata.train_iters,
                                                                loss.detach().cpu()))
        writelogtxt(args.logFile,str(loss.detach().cpu().numpy()),with_time=False) 
        if (epoch+1)%5==0:
    ...
    writelogtxt(args.logFile,"train end at epoch "+str(epoch)) 
    #test on best model
    modelname = args.method_name + '_' + args.data_name + '_valbst' + '.model'
...
class Args():
    def __init__(self):
        self.method_name='Hamida3DCNNbtest_'
        self.lr=0.0005
        self.record=False 
        self.runs=1
        ...
        self.model_save_path='./TBN_others/models'
        self.logFile=self.method_name+self.data_name+'_loss.txt'
        self.recordFile=self.method_name+self.data_name+'_record.txt'

if __name__ == '__main__':
    # reset hyper parameters in the Args 
    seed=np.random.randint(10000)
    args=Args()
    print("args dict:",args.__dict__)
    writelogtxt(args.logFile,"a new start "+'*'*50)
    writelogtxt(args.logFile,str(args.__dict__))
    # run(args)
    # run_more(args)
    test_and_show(test_all_fig=True)
    exit()

    # for ts in [10,15,20,30,40,50,100]:
    #     print('ts ',ts)
    #     args.train_size=ts
    #     run_more(args)
    #     print('ends at ',args.data_name,' with ',args.train_size)
    # exit()
