...
class MyNet(nn.Module):
    ...
    def __init__(self,out_dim,bands,patch_size=1,batch_size=128):
        ...
        self.hidden_size=10
        self.spe_att=
        self.A_RNN=nn.Sequential(
            nn.GRU(self.input_dim,self.hidden_size,1,True,True,bidirectional=True),

        )
    def ARNN(self,x,weights,biases):
        gru_fw_cell=nn.GRUCell(x.shape,num_hidden)
        gru_bw_cell=nn.GRUCell(x.shape,num_hidden)
        output=nn.B

    def forward(self,Xspa,Xspe):
