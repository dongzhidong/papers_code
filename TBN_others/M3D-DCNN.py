...
for indian : eight class, 200 samples per class
97.61%
...
class M3DDCNN(nn.Module):
    ...
    def __init__(self,classnums,bands_num,patch_size=1,batch_size=128):
        ...
        self.pooling=nn.MaxPool2d(kernel_size=(3,2,2),stride=(3,2,2))
...
def test_and_show(mydata=None,colormap=None):
    ...
    # else:
    #     bands=bands_seg_num
    myOneNet = M3DDCNN(args.classnums, bands, args.patch_size, args.batch_size)
    myOneNet.to(device)
...
def run():
    ...
    myNet = M3DDCNN(args.classnums,bands_num, args.patch_size, args.batch_size)
    myNet.to(device)
    ...
    return overall_accuracy,average_accuracy,kp

imgtype='image'
class Args():
    def __init__(self):
        self.method_name='M3DDCNN'
        self.lr=0.01
